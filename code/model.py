...
class LSTMChain(nn.Module):
    ...
    def forward(self, word_embeds, lengths):
        ...
        indexes = (lengths - 1) + torch.arange(batch_size, device=word_embeds.device,
                                               dtype=lengths.dtype) * time_dim  # Index of final states
...
class PyTorchLSTMChain(nn.Module):
    ...
    def forward(self, word_embeds, lengths, dummy_input=False):
        ...
        indexes = (lengths - 1) + torch.arange(batch_size, device=word_embeds.device, dtype=lengths.dtype) * int(
            time_dim)  # Index of final states
...
class ModuleTests():
    ...
    def testBiLSTMReshaping(self):
        output = torch.FloatTensor(
            np.array([[1, 3], [-2, -2], [2, -4], [-6, -1], [0, 0], [0, 0], [1, 2], [3, 4]], dtype=np.float32))
        output = output.reshape(shape=[-1, 2 * output.shape[1]])
        print("Result: " + str(output))

    def testTimePooling(self):
        output = torch.FloatTensor(
            np.array([[[1, 3], [-2, -2], [5, 1]], [[2, -4], [-6, -1], [0, 0]]], dtype=np.float32))
