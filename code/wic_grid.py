...
class WICclassifier(nn.Module):
    def __init__(self, dp1, dp2, batch_size=64, lstm_dim= 400, fc_dim = 100, n_classes = 1):
...
class WIC:
    ...
    def __init__(self, model, dp1, dp2, batch_size=64, lstm_dim= 400, fc_dim = 100, n_classes = 1):
...
#             print("Best split is: ", best_split.item())
    ...
    def mlp_evaluator(self, path, seed, val_type, d1, d2, lr):
        self.encoder.eval()
...
#                 print("NEW best Dev accuracy!!")
    ...
    def WIC_main(self, path, seed, val_type, d1, d2, lrate, iteration=None, dataset=None, ret_pred_list=False):
...
#             print(errors2)
            ...
            return self.mlp_evaluator(path=path, seed=seed, val_type=val_type, d1=d1, d2=d2, lr=lrate)
...
#         wic = WIC(model)
#         wic.WIC_main(val_type = 'threshold')
                ...
                for d1 in drop1:
                    for d2 in drop2:
                        ...
                        for seed in seeds:
                            ...
                            best.append(wic.WIC_main(path = PATH, seed=seed,val_type = val, d1 = d1, d2=d2, lrate= learn))
                        text = "\nAverage over seeds: {:.4f}".format(sum(best)/len(best)*100)
                        text+="\n\n"
                        print(text)
                        p = PATH
                        p = os.path.join(p,"wic_results.txt")
                        with open(p, "a+") as f:
                            f.write(text)
