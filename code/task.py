...
class NLITask(TaskTemplate):
    ...
    def _get_decoder_output(self, embeds_s1, len_premise, embeds_s2, len_hyp):
        out = self.classifier(embeds_s1, len_premise, embeds_s2, len_hyp, applySoftmax=False, decoder_output = True)
        return out
    ...
    def _forward_model(self, embeds, lengths, applySoftmax=False):
        ...
        if not self.classifier.is_word_level():
            out = self.classifier(embed_s1, embed_s2, applySoftmax=applySoftmax, decoder_output = False)
        else:
            out = self.classifier(embed_s1[1], lengths[0], embed_s2[1], lengths[1], applySoftmax=applySoftmax, decoder_output = False)
        return out
